...
\begin{itemize}
    \item For each station we split the data in a part containing the station, that will be used for testing, and another part without the station used for training. This means that we have to train in total $\perplexityinsert{numstations}$ different set of models associated with the different data splits.
...
\end{itemize}
As some stations are too near the border of the image, the available traffic information is not enough to add meaningfull information, for this reason even though we use all stations as information the leave one out for tsting is only performed on the $\perplexityinsert{numstationstest}$ stations \footnote{\perplexityinsert{stationstest}.}.

...
\item \textbf{\perplexityinsert{krigging}}:
\item \textbf{\perplexityinsert{lm}}: We use a Super-learner approach by sequentially combining 
	...
	\item A Graph Feature extractor using traffic information from neighbouring edges up to distance $\perplexityinsert{kneighbours}$, as described in \cref{sec:grpah_features}, with a learnable linear transformation (applying first a z-score normalization and then fitting a Lasso \cite{}) trained to correct the residuals of the previous method in the concatenation.
	\end{enumerate}
	As transition functions for the ssequential supeer-learner we used a linear regression for the first transition and a Lasso for the second.
	
 we first perform a \perplexityinsert{average} our baseline as described in \cref{sec:snapshotmean}.
\item \textbf{\perplexityinsert{lmextra}}:
\item \textbf{\perplexityinsert{nn}}:
\item \textbf{\perplexityinsert{ensemble}}:
\end{itemize}
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figures/plot_barplot_mse_vs_models_by_None.pdf}
    \includegraphics[width=\linewidth]{figures/plot_barplot_mse_vs_station_by_models.pdf}
    \caption{Error plots.}
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figures/plot_pollution_map_in_graph_modelLR_Extra_stationOPERA.pdf}
    \includegraphics[width=\linewidth]{figures/plot_pollution_map_in_graph_modelNN_Extra_stationOPERA.pdf}

...
% \ascomment{[Attempt to systematize and formalize the iterative-LOO, the successive subdivisions and minimization errors.]}

% Split $\cD$: starting from space $X_j^H \subseteq X$ \ascomment{It is not exactly a subset but a concatenation of sub-partitions} with $H$ the set of available times in $X_j^H$  we will construct space $\cD(X_j^H)=X_{j+1}^H\times Y_{j+1}^H$ as the concatenation of its 1-knock-out-element partitions $X_{j+1}^H\times Y_{j+1}^H=^\frown_{s\in S} X_{js}^H \times Y_{js}^H=^\frown_{s\in S}\cD_s(X_j^H)$ \ascomment{which symbol do we use for concatenation?} \ascomment{It is not a tensor product $\times$ but an "alignement", maybe better $(X_{j+1}^H, Y_{j+1}^H)$} where $s\in S_j$ with $S_j$ being the set of available stations in $X_j^H$, $I_s=\{i \in_{i \neq s} S\}$ and
% \begin{align}
%     Y_{js}^H &= (z_s(t), r^\obs_s) \in \bR \times \bR^{2} \, \forall t \in H\\
%     X_{js} &= (z_{I_s}(t), r^\obs_{I_s}, q, T, w) \in \bR^{\#S_j-1} \times \bR^{2(\#S_j-1)} \times V \times \bR\times \bR^2 \, \forall t \in H
% \end{align}
% Then
% \begin{align}
% X^{\test}_s\times Y^{\test}_s &= \cD_s(X^{T_\test})\\
% X^{\train}\times Y^{\train} &= \cD(X^{T_\train})\\
% X^{SL}\times Y^{SL} &= \cD(X^{\train})\\
% X^{M}\times Y^{M} &= \cD(X^{SL})\\
% \end{align}

\begin{enumerate}
	\item \textbf{Test split}: The first split is performed both in time and space. We take the first $\perplexityinsert{percentagetraintime}\%$ of the available times ($\perplexityinsert{numberoftraintimes}$ hours) for training $T_{\train}$ and the rest for testing $T_{\test}$. The Leave-One-Out is done on the station locations $r^\obs$ and associated pollution measurements $z$. As some stations are too close to the border of the image, the available traffic information is not enough to add meaningfull information. For this reason even though we use all stations as information, we only take $\perplexityinsert{numstationstest}$ for perfoming the testing Leave-One-Out\footnote{Stations away from borders are: \perplexityinsert{stationstest}.}. This means that we have to train in total $\perplexityinsert{numstationstest}$ different sets of models each one associated with one of these different testing data splits. Let $m_\test = \perplexityinsert{numstationstest}$, and let
    $$
    I = \{1, \dots, m_\test\}
    $$
    and let
    $$
    [I]^k = \{ S\subseteq I \cond \#S=k \}
    $$
    be the set of all $k$-subsets of $I$ (for $1\leq k \leq m_\test$). Note that we have $\#[I]^k = \binom{m_\test}{k}$ subsets in $I$ and in particular $k=1$ for a Leave-one-out. For any $S\subseteq [I]^k$,
    \begin{align}
    X^{\train}_S(t)
    &= (z_{S^c}(t), r^\obs_{S^c}, q, T, w) \, \quad\forall t \in T_{\train}. \\
    % X^{\test}_S(t)
    % &= (z_{S}(t), r^\obs_{S}, q, T, w) \, \quad\forall t \in T_{\test}. \\
    Y^{\test}_S(t)
    &= (z_{S}(t), r^\obs_{S}) \in \bR \times \bR^{2} \, \forall t \in T_{\test}
    \end{align}
    $$
    e_S(u)
    \approx
    \frac{1}{T_\test %\binom{\perplexityinsert{numstationstest}}{k}
    }
    \sum_{t\in T_\test}
    \Vert z_{S}(t) - A(X_{S^c}^\train)(r_{S}^\obs)\Vert^2
    $$
    \item \textbf{Super-learner train split}: Similarly, for each training set we perform another Leave-One-Out but now we train each super-learner on the concatenation of all the $m_\test-1$ partitions: $(Y^{SL}_{S}, X^{SL}_{S}) = \underset{S'\subset S^c \, : \, \#S'=1}{^\frown}(Y^{SL}_{SS'}, X^{SL}_{SS'})$ both in $X^{\train}_S$.
    \item \textbf{Individual models train split}: For each training super-learner set $(Y^{SL}_{S}, X^{SL}_{S})$, both in $X^{SL}_S$, we perform in a similar way one more Leave-One-Out to train each individual model to avoid the possibility of over-fitting.
\end{enumerate}
...
Following the methodology and using the data explained in the previous sections we put in comparison the following models:
\begin{itemize}
...
\item \textbf{\perplexityinsert{avgkrigging}}: a kernel method with an exponential kernel after subtracting the pollution average along all past times.
...
We can acknowledge that, given the reduced amount of sensors, it is difficult to beat the baseline. However, if the available information is well managed, we can obtain some improvements in the prediction.

We can see in \cref{fig:errorplots} the mean square error on the tested stations. We observe that adding more information, if well managed, leads to improvements in the prediction task. Less clear is the use of non-linearity as the results are mixed in this regard. Overall, methods including traffic information show a mild improvement with respect to the simple average baseline, however, they allow the prediction of local variations of pollution (see \ref{fig:pollutionmaps}).

\perplexityinsert{krigging} and \perplexityinsert{avgkrigging} perform usually bad except for the stations OPERA and HAUS because both are very near to each other (just $\perplexityinsert{operahausdistance}$m appart) and a correlation of $0.87$ which makes kernels methods suitable for this case.

The \perplexityinsert{ensemble} method sometimes shows a mild improvement thanks to the combination but also it shows some catastrophic failures. Overall, the three sequential methods \perplexityinsert{lm}, \perplexityinsert{lmextra} and \perplexityinsert{nn} are the ones that behave the better except for the BONAP adn CELES stations which we believe it is due to the closeness to the river (see \ref{fig:stations-nodes}). Also there is problems in HAUS station due to the presence of the opera icon in the Screenshot image (see \ref{fig:traffic-raw-data}) masking out information about traffic just where the station is placed.

Finally, we should mention that even though the amount of traffic information is big, still, it remains of reduced utility as it measures intensity of traffic but not the amount of actual vehicles that is passing by the street which is the variable that directly impacts the emissions. It can be hoped then that by following a similar approach on better suited data better improvements could be expected.
...
\begin{figure}
    ...
    \caption{Pollution maps. \ascomment{Change color map to autumn}}

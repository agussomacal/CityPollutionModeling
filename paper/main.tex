...
\begin{itemize}
      \item \textbf{Raw data:} Using the Google Maps Platform\footnote{Google Maps Platform: \url{https://mapsplatform.google.com/}, \url{https://developers.google.com/maps/documentation/javascript/trafficlayer}}, we can download the traffic information in the form of an \textit{image} (see Figure \ref{fig:traffic-raw-data}). The colored pixels are the streets where cars are circulating. There are four different colors, and they give a rough estimate of the traffic intensity. We list them in increasing order of intensity as
      $$
      \texttt{colors} = \{ \texttt{green},\, \texttt{orange},\, \texttt{red},\, \texttt{dark-red} \}.
      $$
      We can see in Figure \ref{fig:traffic-raw-data} that the colored traffic areas are arranged following what looks like a street network. Note however that the information at this stage is not given in a graph, but on an image with pixels.  A careful examination reveals that in fact the color locations almost perfectly overlap with the Open Street Maps' metric graph. The task is then to project the information on the graph.
      ...
      \begin{equation}
      q(r,c)=\sum_{\ee\in \E} q_\ee \mathds{1}_\ee(r).
      \label{eq:q}
      \end{equation}
...
\begin{figure}
      ...
      \begin{subfigure}[b]{0.45\textwidth}
          \centering
          \includegraphics[width=\textwidth]{traffic-screenshot-2020-12-29-17-00-01}
          \caption{Raw data from Google Maps: image with four colors.}
          \label{fig:traffic-raw-data}
      \end{subfigure}
      \hfill
      \begin{subfigure}[b]{0.45\textwidth}
          \centering
          \hspace*{-1.9cm}\includegraphics[height=0.25\textheight]{{number_of_cars}}
          \vspace*{-1.5cm}
          \caption{$q(x)$: Projection of the data to $\G$, and weighted average (see \eqref{eq:q}). }
          \label{fig:traffic-cars-on-edge}
      \end{subfigure}
      \caption{Treatment of traffic data. \om{[Update these images.]}}
...
Our approach to reconstruct an unknown pollution field $u\in V$ from the partial data $x_u\in \cX$ is based on a supervised learning strategy involving super-learners. We thus assume that we have a database of $K$ observations
$$
X_K = \{x_1,\dots, x_K\} \subset \cX,
$$
...
\begin{itemize}
\item The set $\cM$ is unknown. Our only information comes from $X_K$ which is connected to a finite set of configurations $u_1, \dots, u_K$ from $\cM$, and it also contains partial information.
...
\end{itemize}
Since computing the best $A$ in the sense of \eqref{eq:best-A} is out of reach, we inevitably have to build sub-optimal mappings. One difficulty that arises is that it is not possible to compare their performance rigorously according to our above quality criteria. Due to this, we propose to develop an approach in which different reconstruction mappings do not compete with each other. Instead, we make them collaborate with each other in a super-learner approach. The strategy relies on nested $n$-fold cross-validation techniques requiring to divide the data base in three disjoint subsets
$$
X_K = X^{\train}_1 \cup X^{\train}_2 \cup X^\val
$$
where $X^{\train}_1$ and $X^{\train}_2$ will be used to train reconstruction mappings, and $X^{\val}$ will serve for out-of-sample validation.
...
% $$
% \tilde e_\infty (A, X_K) \coloneqq \max_{x\in X_K} \tilde e(x, A),
% \qquad
% \tilde e_\av (A, X_K) \coloneqq \frac 1 K \sum_{x\in X_K} \tilde e(x, A).
% $$
% The best reconstruction mapping is then
% $$
% \min_{A: \cX \to V} \tilde e_\star(A, X_K), \quad \star = \{\infty, \av\}.
% $$
...
%\om{[OM to AS: This was a full section before but it seems a bit short. Shall we move it to the section on numerical results for example?]}

To validate the results we can only rely on the available (few) sensors (at most $13$ \verify{13?}) and it does not suffice to make a split of training and testing in time because we observed that models tend to over-fit the locations in which they where trained. For this we need to perform a double Leave-One-Out procedure on the stations:
\begin{itemize}
    \item For each station we split the data in a part containing the station, that will be used for testing, and another part without the station used for training. This means that we have to train in total $13$ \verify{13?} different set of models associated with the different data splits.
    \item For each training set we perform another Leave-One-Out to train each model under different circumstances avoiding this way the possibility of over-fitting.
\end{itemize}
...
\sum_{x\in X^\train_1}
%\sum_{z\in Z^\train}
\Vert z - \cN\cN(\theta)(r^\obs) \Vert^2_2,
$$
we then set
$$
A = \cN\cN(\theta^*).
...
\sum_{x\in X^\train_2}
%\sum_{z\in Z^\train}
\Vert z -  \mathcal{NN} (\theta^*)( c(x,r^\obs) ) \Vert^2_2.
$$
Note that here we use the second training set $X^\train_2$ instead of the first one which is used to train each learner. This is done in order to avoid over-fitting.

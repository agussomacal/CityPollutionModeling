...
class BLUEModel(BaseModel):
    ...
    def __init__(self, name="", sensor_distrust=0.0, loss=mse, optim_method=NONE_OPTIM_METHOD, verbose=False):
        if isinstance(sensor_distrust, (float, int)):
        ...
        super().__init__(name=name, loss=loss, optim_method=optim_method, verbose=verbose,
                         bounds=[(0, 1) for _ in range(len(sensor_distrust))], **sensor_distrust)
